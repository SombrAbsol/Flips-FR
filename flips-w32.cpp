...
bool SelectRom(LPWSTR filename, LPCWSTR title, bool output)
{
	...
	ofn.lpstrFilter=TEXT("Fichiers ROM les plus communs\0*.smc;*.sfc;*.nes;*.gb;*.gbc;*.gba;*.nds;*.vb;*.sms;*.smd;*.md;*.ngp;*.n64;*.z64\0Tous les fichiers (*.*)\0*.*\0");
...
int a_ApplyPatch(LPCWSTR clipatchname)
{
	...
	else
	{
		...
		ofn.lStructSize=sizeof(ofn);
		ofn.hwndOwner=hwndMain;
		ofn.lpstrFilter=TEXT("Tous les patchs supportÃ©s (*.ips, *.bps)\0*.ips;*.bps;*.ups\0Tous les fichiers (*.*)\0*.*\0");
		ofn.lpstrFile=patchnames;
		ofn.nMaxFile=65535;
		ofn.lpstrTitle=TEXT("SÃ©lectionnez les patchs Ã  utiliser");
	...
	if (!multiplePatches)
	{
		...
		if (!inromname)
		{
			...
			inromname_buf[0]='\0';
			if (!SelectRom(inromname_buf, TEXT("SÃ©lectionnez le fichier Ã  patcher"), false)) goto cancel;
		}
		...
		if (!SelectRom(outromname, TEXT("SÃ©lectionnez le fichier de sortie"), true)) goto cancel;
...
#define max(a, b) (a > b ? a : b)
		...
		redo: ;
			...
			LPCSTR messages[8]={
					"Tous les patchs ont été appliqués avec succès !",//e_none
					"Tous les patchs ont été appliqués avec succès !",//e_notice (ignore)
					"Tous les patchs ont été appliqués, mais un ou plusieurs d'entre eux peuvent être brisés ou avoir été créés de manière incorrecte…",//e_warning
					"Certains patchs ont été appliqués, mais ils ne sont pas tous valides…",//e_invalid
					"Certains patchs ont été appliqués, mais les ROMs désirées n'ont pas toutes pu être créées…",//e_rom_io_write
					"Certains patchs ont été appliqués, mais les ROMs renseignées n'ont pas toutes pu être lues…",//e_io_rom_read
					"Certains patchs ont été appliqués, mais les ROMs requises renseignées n'ont pas toutes pu être trouvées…",//e_no_auto
					"Certains patchs ont été appliqués, mais ils n'ont pas tous pu être lus…",//e_io_read_patch
				};
		...
		inromname[0]='\0';
		if (!SelectRom(inromname, TEXT("SÃ©lectionnez le fichier de base"), false)) return 0;
		WCHAR thisFileNameWithPath[MAX_PATH];
		wcscpy(thisFileNameWithPath, patchnames);
		LPWSTR thisFileName=wcschr(thisFileNameWithPath, '\0');
		*thisFileName='\\';
		...
		LPCSTR messages[2][8]={
				{
					...
					NULL,//e_notice
					NULL,//e_warning
					"Aucun de ces patchs n'est valide pour cette ROM !",//e_invalid_this
					"Aucun de ces patchs n'est valide !",//e_invalid
					"Écriture des ROMs impossible. Êtes-vous sur un support en lecture seule ?",//e_io_write
					"Lecture des patchs impossible. Que faites-vous exactement ?",//e_io_read
					"Lecture de la ROM impossible. Que faites-vous exactement ?",//e_io_read_rom
				},{
					//at least one error-free
					"Tous les patchs ont été appliqués avec succès !",//e_none
					"Tous les patchs ont été appliqués avec succès !",//e_notice
					"Tous les patchs ont été appliqués, mais un ou plusieurs d'entre eux peuvent être brisés ou avoir été créés de manière incorrecte…",//e_warning
					"Certains patchs ont été appliqués, mais ils ne sont pas tous valides pour cette ROM…",//e_invalid_this
					"Certains patchs ont été appliqués, mais ils ne sont pas tous valides…",//e_invalid
					"Certains patchs ont été appliqués, mais les ROMs désirées n'ont pas toutes pu être créées…",//e_io_write
					"Certains patchs ont été appliqués, mais ils n'ont pas tous pu être lus…",//e_io_read
					NULL,//e_io_read_rom
				},
			};
		if (inrom.ptr)
...
void a_CreatePatch()
{
	...
	romnames[0][0]='\0';
	romnames[1][0]='\0';
	if (!SelectRom(romnames[0], TEXT("SÃ©lectionnez le fichier ORIGINAL NON-MODIFIÃ‰ Ã  utiliser"), false)) return;
	if (!SelectRom(romnames[1], TEXT("SÃ©lectionnez le NOUVEAU fichier MODIFIÃ‰ Ã  utiliser"), false)) return;
	
	if (!wcsicmp(romnames[0], romnames[1]))
	{
		MessageBoxA(hwndMain, "C'est le même fichier ! Vous devriez vraiment utiliser deux fichiers différents.", flipsversion, mboxtype[el_broken]);
		return;
	}
	...
	ofn.hwndOwner=hwndMain;
	ofn.lpstrFilter =
		TEXT("Fichier de patch BPS (*.bps)\0*.bps\0")
		//TEXT("BPS Patch File (Favor Creation Speed) (*.bps)\0*.bps\0")
		TEXT("Fichier de patch IPS (*.ips)\0*.ips\0");
	ofn.lpstrFile=patchname;
	ofn.nMaxFile=MAX_PATH;
	ofn.nFilterIndex=state.lastPatchType;
	ofn.lpstrTitle=TEXT("SÃ©lectionnez le fichier Ã  enregistrer sous");
...
bool a_SetEmulator()
{
	...
	*newemupath='\0';
	...
	ofn.lStructSize=sizeof(ofn);
	ofn.hwndOwner=hwndMain;
	ofn.lpstrFilter=TEXT("Fichiers de l'Ã©mulateur (*.exe)\0*.exe\0Tous les fichiers (*.*)\0*.*\0");
	ofn.lpstrFile=newemupath;
	ofn.nMaxFile=MAX_PATH;
	ofn.lpstrTitle=TEXT("SÃ©lectionnez l'Ã©mulateur Ã  utiliser");
...
int a_ApplyRun(LPCWSTR clipatchname)
{
	...
	*patchpath='\0';
	...
	else
	{
		...
		ofn.lStructSize=sizeof(ofn);
		ofn.hwndOwner=hwndMain;
		ofn.lpstrFilter=TEXT("Tous les patchs supportÃ©s (*.bps, *.ips)\0*.bps;*.ips;*.ups\0Tous les fichiers (*.*)\0*.*\0");
		ofn.lpstrFile=patchpath;
		ofn.nMaxFile=MAX_PATH;
		ofn.lpstrTitle=TEXT("SÃ©lectionnez le patch Ã  utiliser");
		ofn.Flags=OFN_HIDEREADONLY|OFN_FILEMUSTEXIST|OFN_PATHMUSTEXIST;
	...
	if (!patch)
	{
		errinf=error(el_broken, "Lecture du patch impossible. Que faites-vous exactement ?");
		goto error;
	}
	...
	{
		romname_base[0]='\0';
		if (!SelectRom(romname_base, TEXT("SÃ©lectionnez le fichier de base"), false))
		{
			delete patch;
			return 0;
		}
...
error:
	...
	{
		MessageBoxA(hwndMain, "Impossible d'ouvrir l'mulateur.", flipsversion, mboxtype[el_broken]);
...
#define check(text, w, h, action) \
	...
	line(23);
	firstbutton("Sélectionner l'émulateur", 202/*94*/, 23, 101);
	endline(6);
	
	line(23);
	button("Attribuer les types de fichiers", 143, 23, 102); assocButton=item;
	labelL("(ne peut être annulé)", 98, 13, 0); assocText=item;
	endline(3);
	
	line(13);
	labelL("Lors de l'ouverture par des associations :", 202, 13, 0);
	endline(1);
	line(17);
	radio("Créer la ROM", 79, 17, 103); Button_SetCheck(item, (state.openInEmulatorOnAssoc==false));
	radio("Lancer dans l'émulateur", 140, 17, 104); Button_SetCheck(item, (state.openInEmulatorOnAssoc==true));
	endline(4);
	
	line(17);
	check("Activer le sélecteur automatique de ROM", 217, 17, 105); Button_SetCheck(item, (state.enableAutoRomSelector));
	endline(3);
...
void key_core(bool checkonly, LPCWSTR path, LPCWSTR value, bool * p_hasExts, bool * p_refresh)
{
	...
	DWORD type;
	WCHAR truepath[60];
	wcscpy(truepath, TEXT("Logiciel\\Classes\\"));
...
#define key_touch(path) \
	...
	key("FloatingIPSFileIPS", "Fichier Floating IPS");
	...
	key("FloatingIPSFileBPS", "Fichier Floating IPS");
	...
	{
		SetWindowText(assocText, TEXT("(dÃ©jÃ  fait)"));
...
#define button(x,y,w,h, text) \
	...
	button(6,  6,  110/*77*/,23, "Appliquer un patch"); SetActiveWindow(lastbutton);
	button(124,6,  110/*83*/,23, "Créer un patch");
	button(6,  37, 110/*90*/,23, "Appliquer et lancer");
	button(124,37, 110/*59*/,23, "Paramètres");
