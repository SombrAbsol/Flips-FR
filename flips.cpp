...
const struct errorinfo ipserrors[]={
		{ el_ok, NULL },//ips_ok
		{ el_unlikelythis, "Le patch a été appliqué, mais il n'est très probablement pas destiné à cette ROM." },//ips_notthis
		{ el_unlikelythis, "Le patch a été appliqué, mais il n'a rien fait. Vous avez très probablement déjà le fichier de sortie de ce patch." },//ips_thisout
		{ el_warning, "Le patch a été appliqué, mais il semble brouillé ou malformé." },//ips_suspicious
		{ el_broken, "Le patch est brisé et ne peut pas être utilisé." },//ips_invalid
		
		{ el_broken, "Le format IPS ne prend pas en charge les fichiers de plus de 16 Mo." },//ips_16MB
		{ el_warning, "Les fichiers sont identiques ! Le patch ne va rien modifier." },//ips_identical
	};

const struct errorinfo bpserrors[]={
		{ el_ok, NULL },//bps_ok,
		{ el_notthis, "C'est déjà le fichier de sortie." },//bps_to_output
		{ el_notthis, "Ce patch n'est pas destiné à cette ROM." },//bps_not_this
		{ el_broken, "Ce patch est brisé et ne peut pas être utilisé." },//bps_broken
		{ el_broken, "Lecture du patch impossible." },//bps_io
		
		{ el_warning, "Les fichiers sont identiques ! Le patch ne va rien modifier." },//bps_identical
		{ el_broken, "Ces fichiers sont trop grands pour que ce programme puisse les traiter." },//bps_too_big
		{ el_broken, "Ces fichiers sont trop grands pour que ce programme puisse les traiter." },//bps_out_of_mem (same message as above, it's accurate for both.)
		{ el_broken, "La création du patch a été annulée." },//bps_canceled
	};
...
{
	LPCWSTR header = TEXT("[Flips]\n#La modification de ce fichier peut annuler votre garantie. Ne rapportez pas de bogues si vous le faites.\n");
                                ...
                                LPCWSTR outromname, struct manifestinfo * manifestinfo)
{
	...
	errinf=error(el_broken, "Format de patch inconnu.");
	if (patchtype==ty_bps)
	{
		errinf=bpserrors[bps_apply(patchmem, inrom, &outrom, &manifest, !verifyinput)];
		if (errinf.level==el_notthis && !verifyinput && outrom.ptr)
			errinf = error(el_warning, "Ce patch n'est pas destiné à cete ROM (fichier de sortie créé quand même)");
...
# define z "z"
#endif
				sprintf(errtext, "Ce patch n'est pas destiné à cette ROM. Taille de fichier attendue : %" z "u ; obtenue : %" z "u.", inf.size_in, inrom.len);
			...
			else
			{
				...
				if (inf.crc_in != crc)
				{
					sprintf(errtext, "Ce patch n'est pas destiné à cette ROM. Somme de contrôle attendue : %.8X ; obtenue : %.8X.", inf.crc_in, crc);
					errinf.description=errtext;
				}
			}
		}
	...
	if (errinf.level==el_ok) errinf.description="Le patch a été appliqué avec succès !";
	...
	if (manifestinfo->use)
	{
		if (manifest.ptr)
		{
			...
			{
				if (errinf.level==el_ok) errinf=error(el_warning, "Le patch a été appliqué, mais le manifeste n'a pas pu être créé.");
			}
		}
		else if (manifestinfo->required && errinf.level==el_ok)
		{
			errinf=error(el_warning, "Le patch a été appliqué, mais il n'y avait aucun manifeste présent.");
		}
	}
	...
	if (removeheader)
	{
		...
		if (errinf.level<el_notthis)
		{
			...
			{
				errinf=error(el_broken, "Impossible d'écrire la ROM");
			}
		}
	}
	else if (errinf.level<el_notthis)
	{
		if (!WriteWholeFile(outromname, outrom)) errinf=error(el_broken, "Impossible d'écrire la ROM");
	}
	free(outrom.ptr);
	free(patchmem.ptr);
	...
	if (errinf.level==el_notthis && removeheader)
	{
		...
		if (errinf2.level < el_notthis)
		{
			...
			{
				return error(el_warning, "Le patch a été appliqué, mais il a été créé à partir d'une ROM en-tête, ce qui ne peut pas fonctionner pour tout le monde.");
			}
			else return errinf2;
		}
	}
                               ...
                               LPCWSTR outromname, struct manifestinfo * manifestinfo, bool update_rom_list)
{
	...
	if (!inrom)
	{
		...
		return error(el_broken, "Impossible de lire la ROM");
	}
                            ...
                            LPCWSTR outromname, struct manifestinfo * manifestinfo, bool update_rom_list)
{
	...
	if (!patch)
	{
		return error(el_broken, "Lecture du patch impossible");
	}
...
bool bpsdeltaGetProgress(size_t done, size_t total)
{
	...
	strcpy(bpsdProgStr, "Veuillez patienter… ");
                                  ...
                                  struct manifestinfo * manifestinfo, struct mem * patchmem)
{
	...
	for (int i=0;i<2;i++)
	{
		...
		if (usemmap)
		{
			...
			if (!romsmap[i])
			{
				...
				return error(el_broken, "Impossible de lire cette ROM.");
			}
	...
	if (patchtype==ty_bps || patchtype==ty_bps_linear || patchtype==ty_bps_moremem)
	{
		...
		if (!manifest.ptr) manifesterr=error(el_warning, "Le patch a été créé, mais le manifeste n'a pas pu être lu.");
	}
	else manifesterr=error(el_warning, "Le patch a été créé, mais son format ne supporte pas les manifestes.");
	
	struct errorinfo errinf={ el_broken, "Format de patch inconnu." };
	if (patchtype==ty_ips)
	{
	...
	if (errinf.level==el_ok) errinf.description="Le patch a été créé avec succès !";
	...
	{
		errinf=error(el_warning, "Le patch a été créé, mais la ROM d'entrée est plus grande que "
		                         "la ROM de sortie. Vérifiez si vous les avez bien inversées.");
	}
                             ...
                             struct manifestinfo * manifestinfo, LPCWSTR patchname)
{
	...
	if (errinf.level<el_notthis)
	{
		if (!WriteWholeFile(patchname, patch)) errinf=error(el_broken, "Écriture du patch impossible.");
	}
...
errorlevel patchinfo(LPCWSTR patchname, struct manifestinfo * manifestinfo)
{
	...
	if (!patch)
	{
		puts("Lecture du fichier impossible");
		return el_broken;
	}
	...
	if (patchtype==ty_bps)
	{
		...
		printf("ROM d'entrée : %" z "u octets, CRC32 %.8X", info.size_in, info.crc_in);
		if (inromname) wprintf(TEXT(", %s"), inromname);
		puts("");
		
		printf("ROM de sortie : %" z "u octets, CRC32 %.8X\n", info.size_out, info.crc_out);
	...
	puts("Aucune information disponible pour ce type de patch");
	return el_broken;
}
...
void usage()
{
	...
	// 12345678901234567890123456789012345678901234567890123456789012345678901234567890
	  ...
	  "ou flips [--create] [--exact] [--bps | etc] clean.smc hack.smc [patch.bps]\n"
	  "\n"
	// 12345678901234567890123456789012345678901234567890123456789012345678901234567890
	  "options :\n"
	  "-a --apply : appliquer un patch IPS, BPS ou UPS (par défaut si deux arguments sont donnés)\n"
	  "  si le nom du fichier de sortie n'est pas donné, Flips utilise par défaut patch.smc près du patch\n"
	  "-c --create : créer un patch IPS or BPS (par défaut si trois arguments sont donnés)\n"
	  "-I --info : les fichiers BPS contiennent des informations sur les roms d'entrée et de sortie, les afficher\n"
	  ...
	  "  créer ce format de patch au lieu de le deviner en se basant sur l'extension du fichier\n"
	  "  ignorée lors de l'application\n"
	  " bps creation styles :\n"
	  "  delta est celui par défaut et recommandé ; c'est un bon équilibre entre les\n"
	  "    performances de création et la taille du patch\n"
	  "  delta-moremem est en général un peu plus rapide que delta (~3 %), mais utilise\n"
	  "    deux fois plus de mémoire ; il donne des patchs identiques à delta\n"
	  "  linear est le plus rapide, mails il a tendance à donner d'assez grands patchs\n"
	  "  tous les patcheurs BPS peuvent appliquer tous les styles de patchs, la seule différence est la\n"
	  "    taille de fichier et la performance de création\n"
	  "--exact : ne pas supprimer les en-têtes SMC lors de l'application ou de la création d'un patch BPS\n"
	  "    non recommandé, peut affecter la compatibilité des patcheurs\n"
	  "--ignore-checksum : accepter les erreurs de somme de contrôle (BPS uniquement)\n"
	  "-m ou --manifest : émettre ou insérer un fichier manifeste comme nomderom.xml (BPS uniquement)\n"
	  "-mfilename ou --manifest=nomdefichier : émettre ou insérer un fichier manifeste exactement ici\n"
	...
	switch (action)
	{
		...
		case a_apply_given:
		{
			...
			if (!outname)
			{
				...
				{
					wprintf(TEXT("Vous avez demandÃ© la crÃ©ation du fichier %s, mais ce fichier existe dÃ©jÃ .\n"
					             "Si vous souhaitez le remplacer, fournissez ce nom de fichier explicitement ; sinon, fournissez un autre nom de fichier.\n"),
					             outname);
					return 1;
				}
			}
		...
		case a_create:
		{
			...
			if (!arg[2])
			{
				...
				{
					puts("Erreur : Type de patch inconnu.");
			...
			if (patchtype==ty_null)
			{
				...
				{
					wprintf(TEXT("Erreur : Type de patch inconnu (%s)\n"), patchext);
					return error_to_exit(el_broken);
				}
			}
